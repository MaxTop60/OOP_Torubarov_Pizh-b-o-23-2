...
Группа: ПИЖ-б-0-23-2
## Задача:
1. Дополнить и при необходимости модифицировать приложение, раз-работанное согласно варианту лабораторной работы № 5.
2. Определить в основном классе оператор + для добавления объекта одного из классов-наследников во внутренний массив.
3. Определить в основном классе операторы ++ префиксный и пост-фиксный для добавления объектов по умолчанию в массив (созданных с по-мощью конструктора по умолчанию).
4. Определить в основном классе оператор [] для доступа к эле-менту массива по индексу.
...
## Описание алгоритма
В классе Cinema определены операторы +, ++ и []. Оператор + позволяет добавлять сеансы в кинотеатр, оператор ++ добавляет новый сеанс в кинотеатр (префиксный оператор добавляет стандартный сеанс, а постфиксный оператор добавляет VIP-сеанс), а оператор [] позволяет получить доступ к сеансу по индексу.

В функции main создается объект класса Cinema, добавляются сеансы, выводится информация о кинотеатре и сеансах, а также демонстрируется работа операторов +, ++ и [].

В программе также используется перегрузка операторов для вывода информации о сеансах и кинотеатре.